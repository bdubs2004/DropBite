...
<main class="card">
  ...
  <div id="ok">
    ...
    <p>Your email is confirmed. Tap below to hop back into NiblGo — you'll be signed in automatically.</p>
    <a id="openBtn" class="btn" href="niblgo://auth/confirm">Open NiblGo</a>
...
<script>
  try {
    ...
    // Supabase appends #error=...&error_description=... when a link is expired
    // or already used. Show the recovery message instead of a false "all set".
    if (h.get('error') || h.get('error_code')) {
      ...
      // Forward the session tokens (in the URL fragment) into the app's deep
      // link, so tapping "Open NiblGo" signs the user in and lands them on the
      // home feed instead of the sign-in screen.
      var btn = document.getElementById('openBtn');
      if (btn) btn.href = 'niblgo://auth/confirm' + (location.hash || '');
    }
  } catch (_) {}
</script>
</body>
