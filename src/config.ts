...
export const AI_CONFIG = {
  ...
  systemPrompt: [
    ...
    'write a sensible standard version: usual ingredients with quantities and clear steps.',
    'Only set is_recipe=false when it clearly is not a home-cooked dish (a restaurant',
    'meal, a packaged snack, "just cereal lol"); then leave the rest minimal.',
    'Stay close to the blurb when it spells out ingredients/steps; only fill in a',
    'typical version when it is just a dish name. Quantities may be empty when unknown.',
  ].join(' '),
};
